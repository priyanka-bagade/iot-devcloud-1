...
def placeBoxes(frame, rd):
    ...
    color = (min(int(rd.class_id) * 12.5, 255), min(int(rd.class_id) * 7, 255), min(int(rd.class_id) * 5, 255))
    cv2.rectangle(frame, (int(rd.xmin), int(rd.ymin)), (int(rd.xmax), int(rd.ymax)), color, 2)
    cv2.putText(frame, rd.det_label + ' ' + str(round(float(rd.prob), 1)) + ' %', (int(rd.xmin), int(rd.ymin) - 7), cv2.FONT_HERSHEY_COMPLEX, 0.6, color, 1)
